...
class EventComponent:
    ...
    def render(self, pdf: FPDF, theme: PDFTheme, event: Event):
        ...
        pdf.cell(0, 10, heading, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        # Details
        pdf.set_font(theme.fonts["body"], "", 10)
        if event.kind:
            pdf.cell(0, 5, f"Kind: {event.kind}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        if event.location:
            pdf.cell(0, 5, f"Location: {event.location}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        if event.time_utc:
            pdf.cell(0, 5, f"Time (UTC): {event.time_utc}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        if event.who:
            pdf.cell(0, 5, f"Who: {', '.join(event.who)}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        if event.travel_to:
            ...
            pdf.cell(0, 5, "Travel To:", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font(theme.fonts["body"], "", 10)
