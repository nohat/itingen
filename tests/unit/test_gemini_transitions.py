...
def test_gemini_transition_hydrator_fallback_generic():
    """Test fallback when Gemini API fails with generic strategy."""
    ...
    # Should fall back to generic transition
    assert result[1].transition_from_prev == "Move from Auckland Ferry Terminal to Rotorua."


def test_gemini_transition_hydrator_fallback_skip():
    """Test fallback when Gemini API fails with skip strategy."""
    client = Mock(spec=GeminiClient)
    client.generate_text.side_effect = Exception("API error")
    ...
    # Should skip transition (no fallback)
    assert result[1].transition_from_prev is None


def test_gemini_transition_hydrator_fallback_error():
    """Test fallback when Gemini API fails with error strategy."""
    client = Mock(spec=GeminiClient)
    client.generate_text.side_effect = Exception("API error")
    ...
    with pytest.raises(ValueError, match="Failed to generate transition"):
